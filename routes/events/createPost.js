...
router.patch('/post', verify(['admin','user']), async (req,res,next)=>{
  ...
  if(!userInEvent.length){
    ...
    return next(new AppError('You cant post into this event', 404));
  }
  ...
  try{ 
    ...
    if(thisEvent.posts.includes(post._id)){
      return next(new AppError('post already created', 404));
    ...
    return next(new AppError('Failed to post', 404));
  }
});
